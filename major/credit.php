<?php
   if(isset($_POST['submit'])){
    ...
    $sql = "select * from `creditpay_data` where name='$name'";
    $result = mysqli_query($conn, $sql);
    $count_name = mysqli_num_rows($result);

    $sql = "select * from `creditpay_data` where cardnumber ='$cardnumber'";
    $result = mysqli_query($conn, $sql);
    $count_cardnumber = mysqli_num_rows($result);

    $sql = "select * from `creditpay_data` where month='$month'";
    $result = mysqli_query($conn, $sql);
    $count_month = mysqli_num_rows($result);

    $sql = "select * from `creditpay_data` where year='$year'";
    $result = mysqli_query($conn, $sql);
    $count_year = mysqli_num_rows($result);

    $sql = "select * from `creditpay_data` where cvv='$cvv'";
    $result = mysqli_query($conn, $sql);
    $count_cvv = mysqli_num_rows($result);
    
   
    $sql = "insert into creditpay_data(name, cardnumber, month, year, cvv) values('$name', '$cardnumber' , '$month','$year','$cvv')";
    $result = mysqli_query($conn, $sql);
    echo '<script>
...
<body>
    ...
    <section class="payment-form">
      ...
      <form id="credit-card-form" action="credit.php" method="POST">
